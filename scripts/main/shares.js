...
const PoolShares = function (logger, client, poolConfig, portalConfig) {
  ...
  const _this = this;
  // Note: process.setMaxListeners(0) is not needed now that we removed duplicate listeners.
  // It is kept for backward compatibility but does not affect the fix.
  ...
  // ------------------------------------------------------------------------
  // FIX: Removed duplicate 'ready', 'error', and 'end' listeners.
  // These are now attached only once inside PoolDatabase.buildRedisClient().
  // That eliminates the "MaxListenersExceededWarning" entirely.
  // ------------------------------------------------------------------------
